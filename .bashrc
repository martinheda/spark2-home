...
# nvm first so Node 20 + global bins (e.g. bw) come from nvm
export NVM_DIR="$HOME/.nvm"
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"  # This loads nvm
[ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"  # This loads nvm bash_completion

# .npm-global after nvm so nvm's bw/node take precedence
export PATH="$PATH:$HOME/.npm-global/bin"

eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv bash)"
. "$HOME/.cargo/env"

# Ensure nvm's bin is first so bw/node come from nvm (brew/cargo may have prepended)
nvm use 20 2>/dev/null || true
setxkbmap se
export PATH="$HOME/.local/bin:$PATH"

# OpenClaw Completion
source "/home/martin/.openclaw/completions/openclaw.bash"
